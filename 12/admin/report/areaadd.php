...
area_add.FormID = "fareaadd"; // form ID
var EW_PAGE_ID = area_add.PageID; // for backward compatibility

// extend page with ValidateForm function
area_add.ValidateForm = function(fobj) {
...
area_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
area_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p class="phpmaker ewTitle"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $area->TableCaption() ?></p>
<p class="phpmaker"><a href="<?php echo $area->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php
$area_add->ShowMessage();
?>
<form name="fareaadd" id="fareaadd" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return area_add.ValidateForm(this);">
<p>
...
<input type="text" name="x_area_name" id="x_area_name" size="30" maxlength="30" value="<?php echo $area->area_name->EditValue ?>"<?php echo $area->area_name->EditAttributes() ?>>
...
class carea_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			$area->CurrentAction = $_POST["a_add"]; // Get form action
			$this->LoadOldRecord(); // Load old recordset
			$this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$area->CurrentAction = "I"; // Form error, reset action
			...
			if (@$_GET["area_name"] != "") {
				...
				$area->setKey("area_name", $area->area_name->CurrentValue); // Set up key
			} else {
				$area->setKey("area_name", ""); // Clear key
				$bCopy = FALSE;
			}
			if ($bCopy) {
				$area->CurrentAction = "C"; // Copy record
			} else {
				$area->CurrentAction = "I"; // Display blank record
		...
		switch ($area->CurrentAction) {
			...
			case "A": // ' Add new record
				$area->SendEmail = TRUE; // Send email on add success
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					$this->setSuccessMessage($Language->Phrase("AddSuccess")); // Set up success message
					$sReturnUrl = $area->getReturnUrl();
					if (ew_GetPageName($sReturnUrl) == "areaview.php")
						$sReturnUrl = $area->ViewUrl(); // View paging, return to view page with keyurl directly
		...
		$area->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $area;
		$area->aid->CurrentValue = NULL;
		$area->aid->OldValue = $area->aid->CurrentValue;
		$area->sid->CurrentValue = NULL;
		$area->sid->OldValue = $area->sid->CurrentValue;
		$area->cid->CurrentValue = NULL;
		$area->cid->OldValue = $area->cid->CurrentValue;
		$area->area_name->CurrentValue = NULL;
		$area->area_name->OldValue = $area->area_name->CurrentValue;
	}
	...
	function RenderRow() {
		...
		if ($area->RowType == EW_ROWTYPE_VIEW) { // View row
		...
		} elseif ($area->RowType == EW_ROWTYPE_ADD) { // Add row
			...
			$area->area_name->EditValue = ew_HtmlEncode($area->area_name->CurrentValue);
	...
	function AddRow($rsold = NULL) {
		...
		if ($area->area_name->CurrentValue == "" && $area->area_name->getSessionValue() == "") {
		...
		// area_name
		$area->area_name->SetDbValueDef($rsnew, $area->area_name->CurrentValue, "", FALSE);

		// Call Row Inserting event
		$rs = ($rsold == NULL) ? NULL : $rsold->fields;
		$bInsertRow = $area->Row_Inserting($rs, $rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$AddRow = $conn->Execute($area->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
