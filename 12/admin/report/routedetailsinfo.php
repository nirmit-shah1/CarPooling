...
//
// Table class for routedetails
//
class croutedetails {
	var $TableVar = 'routedetails';
	...
	//
	function croutedetails() {
		global $Language;
		$AllowAddDeleteRow = ew_AllowAddDeleteRow(); // Allow add/delete row

		// reg_id
		$this->reg_id = new cField('routedetails', 'routedetails', 'x_reg_id', 'reg_id', '`reg_id`', 3, -1, FALSE, '`reg_id`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->reg_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['reg_id'] =& $this->reg_id;

		// mid
		$this->mid = new cField('routedetails', 'routedetails', 'x_mid', 'mid', '`mid`', 3, -1, FALSE, '`mid`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->mid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['mid'] =& $this->mid;

		// source
		$this->source = new cField('routedetails', 'routedetails', 'x_source', 'source', '`source`', 201, -1, FALSE, '`source`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['source'] =& $this->source;

		// destination
		$this->destination = new cField('routedetails', 'routedetails', 'x_destination', 'destination', '`destination`', 201, -1, FALSE, '`destination`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['destination'] =& $this->destination;

		// intermediatedestination1
		$this->intermediatedestination1 = new cField('routedetails', 'routedetails', 'x_intermediatedestination1', 'intermediatedestination1', '`intermediatedestination1`', 201, -1, FALSE, '`intermediatedestination1`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['intermediatedestination1'] =& $this->intermediatedestination1;

		// intermediatedestination2
		$this->intermediatedestination2 = new cField('routedetails', 'routedetails', 'x_intermediatedestination2', 'intermediatedestination2', '`intermediatedestination2`', 201, -1, FALSE, '`intermediatedestination2`', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `routedetails` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE `routedetails` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `routedetails` WHERE ";
		$SQL .= ew_QuotedName('mid') . '=' . ew_QuotedValue($rs['mid'], $this->mid->FldDataType) . ' AND ';
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->mid->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@mid@", ew_AdjustSql($this->mid->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("routedetailsview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "routedetailsadd.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("routedetailsedit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("routedetailsadd.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("routedetailsdelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->mid->CurrentValue)) {
			$sUrl .= "mid=" . urlencode($this->mid->CurrentValue);
		} else {
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=routedetails" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function LoadListRowValues(&$rs) {
		...
		$this->mid->setDbValue($rs->fields('mid'));
   ...
   // Common render codes
		...
		// mid
		$this->mid->LinkCustomAttributes = "";
		$this->mid->HrefValue = "";
		$this->mid->TooltipValue = "";
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$XmlDoc->AddField('mid', $this->mid->ExportValue($this->Export, $this->ExportOriginalValue));
