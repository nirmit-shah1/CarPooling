...
// Create page object
$member_signup_edit = new cmember_signup_edit();
$Page =& $member_signup_edit;

// Page init
$member_signup_edit->Page_Init();

// Page main
$member_signup_edit->Page_Main();
?>
...
// Create page object
var member_signup_edit = new ew_Page("member_signup_edit");

// page properties
member_signup_edit.PageID = "edit"; // page ID
member_signup_edit.FormID = "fmember_signupedit"; // form ID
var EW_PAGE_ID = member_signup_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
member_signup_edit.ValidateForm = function(fobj) {
...
member_signup_edit.Form_CustomValidate =  
...
member_signup_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
member_signup_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<?php $member_signup_edit->ShowPageHeader(); ?>
<p class="phpmaker ewTitle"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $member_signup->TableCaption() ?></p>
<p class="phpmaker"><a href="<?php echo $member_signup->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php
$member_signup_edit->ShowMessage();
?>
<form name="fmember_signupedit" id="fmember_signupedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return member_signup_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="member_signup">
...
<div<?php echo $member_signup->reg_id->ViewAttributes() ?>><?php echo $member_signup->reg_id->EditValue ?></div>
<input type="hidden" name="x_reg_id" id="x_reg_id" value="<?php echo ew_HtmlEncode($member_signup->reg_id->CurrentValue) ?>">
...
<?php
$member_signup_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script language="JavaScript" type="text/javascript">
...
<?php
$member_signup_edit->Page_Terminate();
?>
<?php
...
class cmember_signup_edit {
	...
	// Page object name
	var $PageObjName = 'member_signup_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$member_signup->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$member_signup->CurrentAction = ""; // Form error, reset action
				$this->setFailureMessage($gsFormError);
				$member_signup->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$member_signup->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($member_signup->reg_id->CurrentValue == "")
			$this->Page_Terminate("member_signuplist.php"); // Invalid key, return to list
		switch ($member_signup->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$member_signup->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$member_signup->RowType = EW_ROWTYPE_EDIT; // Render as Edit
	...
	function RenderRow() {
		...
		} elseif ($member_signup->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// reg_id
			$member_signup->reg_id->EditCustomAttributes = "";
			$member_signup->reg_id->EditValue = $member_signup->reg_id->CurrentValue;
			$member_signup->reg_id->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $member_signup->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				if (count($rsnew) > 0)
					$EditRow = $conn->Execute($member_signup->UpdateSQL($rsnew));
				else
		...
		if ($EditRow)
			$member_signup->Row_Updated($rsold, $rsnew);
		$rs->Close();
		return $EditRow;
	}
