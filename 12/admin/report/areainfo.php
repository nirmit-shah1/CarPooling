...
class carea {
	...
	function carea() {
		...
		// aid
		$this->aid = new cField('area', 'area', 'x_aid', 'aid', '`aid`', 3, -1, FALSE, '`aid`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->aid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['aid'] =& $this->aid;

		// sid
		$this->sid = new cField('area', 'area', 'x_sid', 'sid', '`sid`', 3, -1, FALSE, '`sid`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->sid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['sid'] =& $this->sid;

		// cid
		$this->cid = new cField('area', 'area', 'x_cid', 'cid', '`cid`', 3, -1, FALSE, '`cid`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->cid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['cid'] =& $this->cid;

		// area_name
		$this->area_name = new cField('area', 'area', 'x_area_name', 'area_name', '`area_name`', 200, -1, FALSE, '`area_name`', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `area` ($names) VALUES ($values)";
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("areaview.php", $this->UrlParm());
	}
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=area" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["area_name"]; // area_name
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					$XmlDoc->AddField('aid', $this->aid->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('sid', $this->sid->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('cid', $this->cid->ExportValue($this->Export, $this->ExportOriginalValue));
